...
if ( function_exists( 'innofitp_activate' ) ):
		...
		if($innofit_data) 
		{
			foreach($innofit_data as $key=>$innofit_value)
			{	
                do_action( 'innofit_before_'.$innofit_value.'_section_hook', false);
				
				do_action( 'innofit_'.$innofit_value.'_action', false);
				
				do_action( 'innofit_after_'.$innofit_value.'_section_hook', false);
